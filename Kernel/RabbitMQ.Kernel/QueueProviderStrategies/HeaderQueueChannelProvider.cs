...
{
    public class HeaderQueueChannelProvider<TQueueMessage> : QueueChannelProviderBase<TQueueMessage>, IQueueChannelProvider<TQueueMessage> where TQueueMessage : IQueueMessage
    {
        private Dictionary<string, object> _headers;

        public HeaderQueueChannelProvider(IChannelProvider channelProvider) : base(channelProvider)
        {

        }

        public void setheader(Dictionary<string, object> headers)
        {
            //to be implemented
            _headers = headers;
        }
        public override void DeclareQueueAndDeadLetter(string _queueName = null)
        {
            try
            {
                ...
                base._channel.ExchangeDeclare(exchange: deadLetterQueueName, type: ExchangeType.Headers);
                ...
                base._channel.ExchangeDeclare(exchange: _queueName, type: ExchangeType.Headers);

                base._channel.QueueDeclare(queue: _queueName, durable: true, exclusive: false, autoDelete: false, arguments: queueArgs);

                base._channel.QueueBind(queue: _queueName, exchange: _queueName, routingKey: "", arguments: _headers);
