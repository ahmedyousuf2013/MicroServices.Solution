...
{
    public class DirectQueueProducer<TQueueMessage> : QueueProducerBasic<TQueueMessage>, IQueueProducer<TQueueMessage> where TQueueMessage : IQueueMessage
    {
        public DirectQueueProducer(ILogger<QueueProducerBasic<TQueueMessage>> logger
            , IQueueNameResolver queueNameResolver
            , IServiceProvider serviceProvider) : base(serviceProvider.GetRequiredService<DirectQueueChannnelProvider<TQueueMessage>>()
                , logger
                , queueNameResolver)
        {
            ...
            base._queueName = $"{Setting._queueNameDirect}{queueNameResolver.Resolve<TQueueMessage>()}" ?? typeof(TQueueMessage).Name;
        }
        ...
        public override void PublishMessage(TQueueMessage message)
        {
            ...
            try
            {
                ...
                channel.BasicPublish(exchange: _queueName, routingKey: Setting._direct_init, true, null, body: serializedMessage);
            }
            catch (Exception ex)
            {
